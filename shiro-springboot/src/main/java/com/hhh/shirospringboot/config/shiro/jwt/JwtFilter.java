...
import com.hhh.shirospringboot.util.common.JsonConvertUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
  ...
  private boolean refreshToken(ServletRequest request, ServletResponse response) {
    ...
    if (RedisHandle.hasKey(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
      ...
      if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
        ...
        Long refreshTokenExpireTime = Constant.REFRESH_TOKEN_EXPIRE_TIME;
        // 设置RefreshToken中的时间戳为当前最新时间戳，且刷新过期时间重新为30分钟过期(配置文件可配置refreshTokenExpireTime属性)
        RedisHandle.set(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis, refreshTokenExpireTime);
